...
func AddSSHPublicKey(image string) error {
	...
	mkdirCmd := exec.Command("/usr/bin/virt-customize", "--no-selinux-relabel", "-a", image, "--mkdir", "/root/.ssh", "--root-password", "password:root", "--password", "password:password", "--network", "--ssh-inject", "ubuntu:file:/root/.ssh/id_rsa.pub", "--ssh-inject", "root:file:/root/.ssh/id_rsa.pub")
