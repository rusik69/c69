...
deploy:
	...
	ssh x220.rusik69.lol "for i in {0..10}; do sudo virsh destroy test$i; sudo virsh undefine test$i; done" || true
	...
	ssh x230.rusik69.lol "for i in {0..10}; do sudo virsh destroy test$i; sudo virsh undefine test$i; done" || true
